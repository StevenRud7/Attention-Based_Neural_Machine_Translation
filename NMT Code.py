...
class ScaledDotAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        k = self.K(keys)
        v = self.V(values)
        unnormalized_attention = torch.bmm(q, k.transpose(1, 2))
        attention_weights = F.softmax(unnormalized_attention / np.sqrt(k.size(-1)), dim=2)
...
class CausalScaledDotAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        mask = torch.triu(self.neg_inf.repeat(keys.size(1), keys.size(1)), diagonal=1) #Mask
        unnormalized_attention = unnormalized_attention + mask.unsqueeze(0)
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, inputs, annotations, hidden_init):
        ...
        for i in range(self.num_layers):
          ...
          new_contexts, encoder_attention_weights = self.encoder_attentions[i](residual_contexts, annotations, annotations) # Encoder attention layer
          residual_contexts = residual_contexts + new_contexts
    
          new_contexts = self.attention_mlps[i](residual_contexts.view(-1, self.hidden_size)).view(batch_size, seq_len, self.hidden_size) # Feedforward layer
